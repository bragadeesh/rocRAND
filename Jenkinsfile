...
properties([buildDiscarder(logRotator(
    ...
    // parameters([booleanParam( name: 'push_image_to_docker_hub', defaultValue: false, description: 'Push rocrand image to rocm docker-hub' )]),
...
Boolean docker_build_inside_image( def build_image, compiler_data compiler_args, docker_data docker_args, project_paths paths )
{
  ...
  // For the nvidia path, we somewhat arbitrarily choose to use the hcc-ctu rocrand package
  String rocrand_archive_path=compiler_args.compiler_name;
  if( rocrand_archive_path.toLowerCase( ).startsWith( 'nvcc-' ) )
  {
    rocrand_archive_path='hcc-ctu'
  }
    ...
    stage( "Test ${compiler_args.compiler_name} ${compiler_args.build_config}" )
    {
      ...
      if( compiler_args.compiler_name.toLowerCase( ).startsWith( 'hcc-' ) )
      {
        ...
        sh  """#!/usr/bin/env bash
            ...
            mv ${paths.project_build_prefix}/build/*.deb ${docker_context}
            mv ${paths.project_build_prefix}/build/*.rpm ${docker_context}
...
// String docker_test_install( String hcc_ver, String artifactory_org, String from_image, String rocrand_src_rel, String build_dir_rel )
String docker_test_install( compiler_data compiler_args, docker_data docker_args, project_paths rocrand_paths, String job_name )
{
  def rocrand_install_image = null
  String image_name = "rocrand-hip-${compiler_args.compiler_name}-ubuntu-16.04"
  String docker_context = "${compiler_args.build_config}/${compiler_args.compiler_name}"
  ...
  stage( "Artifactory ${compiler_args.compiler_name} ${compiler_args.build_config}" )
  {
    ...
    sh  """#!/usr/bin/env bash
        ...
        cp -r ${rocrand_paths.project_src_prefix}/docker/* ${docker_context}
      """

    // Docker 17.05 introduced the ability to use ARG values in FROM statements
    // Docker inspect failing on FROM statements with ARG https://issues.jenkins-ci.org/browse/JENKINS-44836
    // rocrand_install_image = docker.build( "${job_name}/${image_name}:${env.BUILD_NUMBER}", "--pull -f ${build_dir_rel}/dockerfile-rocrand-ubuntu-16.04 --build-arg base_image=${from_image} ${build_dir_rel}" )
    ...
    rocrand_install_image = docker.image( "${job_name}/${image_name}" )
  }
...
// {
//   println "Enabling rocrand integration testing pass"

//   node('docker && rocm')
//   {
//     rocrand_integration_testing( '--device=/dev/kfd', 'hip-ctu', 'Release' )
//   }
...
def build_pipeline( compiler_data compiler_args, docker_data docker_args, project_paths rocrand_paths, def docker_inside_closure )
{
  ansiColor( 'vga' )
  {
    ...
    stage( "Build ${compiler_args.compiler_name} ${compiler_args.build_config}" )
    {
      ...
      checkout_and_version( rocrand_paths )

      // Conctruct a binary directory path based on build config
      build_directory_rel( rocrand_paths, compiler_args );

      // Create/reuse a docker image that represents the rocrand build environment
      def rocrand_build_image = docker_build_image( docker_args, rocrand_paths )

      // Print system information for the log
      rocrand_build_image.inside( docker_args.docker_run_args, docker_inside_closure )

      // Build rocrand inside of the build environment
      build_succeeded = docker_build_inside_image( rocrand_build_image, compiler_args, docker_args, rocrand_paths )
    }
    ...
    if( compiler_args.compiler_name.toLowerCase( ).startsWith( 'hcc-' ) )
    {
      ...
      String rocrand_image_name = docker_test_install( compiler_args, docker_args, rocrand_paths, job_name )

      docker_clean_images( job_name, rocrand_image_name )
    }
  }
}
...
parallel hcc_ctu:
{
  node( 'docker && rocm' )
  {
    def docker_args = new docker_data(
        from_image:'compute-artifactory:5001/rocm-docker-github/rocm-developer-tools/hip/master/hip-hcc-ctu-ubuntu-16.04:337',
    ...
    def rocrand_paths = new project_paths(
